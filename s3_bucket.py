...
def read_file(bucket_name, file_name):
    client = create_client()
    try:
        file = client.get_object(Bucket=bucket_name, Key=file_name)['Body'].read()
        ...
        if 'Contents' in response:
            for item in response['Contents']:
                ...
                while response['KeyCount'] == 1000:
                    response = client.list_objects_v2(Bucket=bucket_name, StartAfter=response['Contents'][0]['Key'],)
                    for item in response['Contents']:
                        print('deleting file', item['Key'])
                        client.delete_object(Bucket=bucket_name, Key=item['Key'])
...
read_file(bucket_name, file_name)
download_file(bucket_name, file_name, file_name)
delete_file(bucket_name, file_name)
empty_s3_bucket(bucket_name)
delete_bucket(bucket_name)

url_link = create_presigned_url(bucket_name, file_name, expiration=36000)
print(url_link)
